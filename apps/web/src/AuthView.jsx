...
export default function AuthView({ onAuthed }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const body = mode === "register" ? { email, password, plan } : { email, password };
  ...
  return (
    <div style={{ maxWidth: 460, margin: "48px auto", padding: 16 }}>
      <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 12 }}>RankyPulse</h2>

      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <button onClick={() => setMode("login")} style={{ padding: "8px 10px" }}>Login</button>
        <button onClick={() => setMode("register")} style={{ padding: "8px 10px" }}>Register</button>
      </div>
      ...
      <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        ...
        <button type="submit" style={{ padding: 10, fontWeight: 800 }}>
