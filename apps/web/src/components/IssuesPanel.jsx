...
export default function IssuesPanel({ issues = [] }) {
  ...
  const impactOptions = useMemo(() => {
    const set = new Set();
    for (const it of issues || []) for (const tag of asArray(it?.impact)) set.add(tag);
  ...
  const summary = useMemo(() => {
    ...
    return out;
  }, [issues, q, priority, severity, impact]);
  ...
  return (
    <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center gap-3">
        <div className="text-sm font-semibold text-slate-900">Issues</div>
        <div className="text-xs text-slate-500">{filtered.length} shown • {issues.length} total</div>
