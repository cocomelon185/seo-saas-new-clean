...
function makeStore(rootDir) {
  ...
  function summarize(result) {
    ...
    const issue_ids = issues.map(x => x && x.issue_id).filter(Boolean);
    const issue_key = issues.map(x => (x && (x.issue_id || x.title || x.name)) ? String(x.issue_id || x.title || x.name) : null).filter(Boolean);
    return { score, quick_wins, issues_count: issues.length, issue_ids, issue_key };
  }
